...
require("dotenv").config(); // Load private credentials from .env
...
const ENERGY_DATA_URL = `https://portal.solaranalytics.com.au/api/v3/live_site_data?site_id=${process.env.SOLAR_SITE_ID}&last_six=false&last_hour=false&battery_data=false`;

// Load credentials from .env file
const PRIVATE_USERNAME = process.env.SOLAR_USERNAME;
const PRIVATE_PASSWORD = process.env.SOLAR_PASSWORD;
const PORT = process.env.PORT || 3000;

// Function to get private API token
async function getPrivateToken() {
    ...
    try {
        ...
        return data.token; // Return the personal API token
    } catch (error) {
...
// GET Route to Fetch Energy Data
app.get("/get-energy-data", async (req, res) => {
    ...
    try {
        ...
        const data = await response.json();
        res.json(data); // Send energy data to frontend
